...
static const struct deck_driver ** drivers; // @@ RevEng: an array of deck_driver structs?
...
static void deckdriversInit() {
  ...
  if (!init) {
    ...
    driversLen = &_deckDriver_stop - &_deckDriver_start; // @@ RevEng: so the linker has defined these somehow?
    init = true;

    // @@ RevEng: Printing the VID and PIDs of the attached decks.
