...
bool deckTest()
{
  ...
  for (i=0; i<nDecks; i++) {
    ...
    // @@ RevEng: Seems to be replicated by esp-drone in sensors_mpu6050_hm5883L_ms5611.c but
    // the decks are hardcoded through ifdef statements
    // => Add one of those ifdef cases for the multiranger!
    if (deck->driver->test) {
      if (deck->driver->test()) {
