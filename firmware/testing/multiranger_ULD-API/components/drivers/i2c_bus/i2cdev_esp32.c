                    ...
                    uint16_t len, uint8_t *data)
{
    ...
    if (memAddress != I2CDEV_NO_MEM_ADDR) {
        ...
        i2c_master_write_byte(cmd, (devAddress << shift) | I2C_MASTER_WRITE, I2C_MASTER_ACK_EN);
        i2c_master_write_byte(cmd, memAddress, I2C_MASTER_ACK_EN);
    }
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (devAddress << shift) | I2C_MASTER_READ, I2C_MASTER_ACK_EN); // @@ removed bit shift
                     ...
                     uint16_t len, uint8_t *data)
{
    ...
    if (memAddress != I2C_NO_INTERNAL_ADDRESS) {
        ...
        i2c_master_write_byte(cmd, (devAddress << shift) | I2C_MASTER_WRITE, I2C_MASTER_ACK_EN); // @@ because 0x52/53
    ...
    i2c_master_write_byte(cmd, (devAddress << shift) | I2C_MASTER_READ, I2C_MASTER_ACK_EN);
    i2c_master_read(cmd, data, len, I2C_MASTER_LAST_NACK);
    i2c_master_stop(cmd);
                     ...
                     uint16_t len, uint8_t *data)
{
    ...
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (devAddress << shift) | I2C_MASTER_WRITE, I2C_MASTER_ACK_EN); // @@ bit shift removed
    if (memAddress != I2CDEV_NO_MEM_ADDR) {
        i2c_master_write_byte(cmd, memAddress, I2C_MASTER_ACK_EN);
    }
