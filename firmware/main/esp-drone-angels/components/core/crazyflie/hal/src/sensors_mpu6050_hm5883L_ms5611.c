...
#include "zranger2.h"
#include "vl53l1x.h"
#include "mmc5983ma.h" // @@ added
#include "flowdeck_v1v2.h"
#include "multiranger.h" // @@ added
    ...
    while (1) {
        ...
        if (pdTRUE == xSemaphoreTake(sensorsDataReady, portMAX_DELAY)) {
            ...
            if (isMagnetometerPresent) {
                processMagnetometerMeasurements(&(buffer[SENSORS_MPU6050_BUFF_LEN])); // @@ starting from the end of the imu data
            }
...
{
    // @@ which buffer is this? the local or global? local makes more sense but this seems messy
    // @@ Get the data out of the slave data register (see how the slave data register was set up in the sensorsSetupSlave function)
    // @@ no way to check if the data is actually ready with the mmc (there is a ready status but its not a contiguous block with the other stuff)
   
    #ifdef SENSORS_ENABLE_MAG_MMC5983MA
        int16_t headingx = (((int16_t)buffer[0]) << 8) | buffer[1];
        int16_t headingy = (((int16_t)buffer[4]) << 8) | buffer[5];
        int16_t headingz = (((int16_t)buffer[2]) << 8) | buffer[3];

        sensorData.mag.x = (float)headingx / MAG_GAUSS_PER_LSB; // @@ convert to gauss
        sensorData.mag.y = (float)headingy / MAG_GAUSS_PER_LSB;
        sensorData.mag.z = (float)headingz / MAG_GAUSS_PER_LSB;
    #elif SENSORS_ENABLE_MAG_HM5883L
...
#endif
#ifdef SENSORS_ENABLE_MAG_MMC5983MA
    mmc5983maInit(I2C0_DEV);

    if (mmc5983maTestConnection() == true) {
        isMagnetometerPresent = true;
        ///hmc5883lSetMode(HMC5883L_MODE_CONTINUOUS); // 16bit 100Hz
        DEBUG_PRINTI("mmc5983ma I2C connection [OK].\n");
    } else {
        DEBUG_PRINTW("mmc5983ma I2C connection [FAIL].\n");
    }
...
//@@ Modification
#ifdef SENSORS_ENABLE_MULTIRANGER
    mrInit();

    if (mrTest() == true) {
        DEBUG_PRINTI("MultiRanger I2C connection [OK].\n");
    } else {
        DEBUG_PRINTW("MultiRanger I2C connection [FAIL].\n");
    }
#endif
...
static void sensorsSetupSlaveRead(void)
{
    ...
    if (isMagnetometerPresent) {
        // Set registers for mpu6050 master to read from
        mpu6050SetSlaveAddress(0, 0x80 | MMC5983MA_ADDRESS);        // set the magnetometer to Slave 0, enable read (0x80)
        mpu6050SetSlaveRegister(0, MMC5983MA_RA_XOUT0);       // read the magnetometer heading register
        mpu6050SetSlaveDataLength(0, SENSORS_MAG_BUFF_LEN); // @@ mmc5983ma: x[17:10] x[10:2] y[17:10] y[10:2] z[17:10] z[10:2] xyz[1:0,1:0,1:0] hmc5883l:model,x,z,y,status ak89663:read 8 bytes (ST1, x, y, z heading, ST2 (overflow check))
        mpu6050SetSlaveDelayEnabled(0, true);
        mpu6050SetSlaveEnabled(0, true);
        DEBUG_PRINTD("mpu6050SetSlaveAddress MMC5983MA done \n");
    }
